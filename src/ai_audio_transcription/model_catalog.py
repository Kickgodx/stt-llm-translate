...
class ModelOption:
    ...
    def display(self) -> str:
        return f"{self.label} ({self.id})" if self.label else self.id
...
def stt_dropdown_labels() -> list[str]:
    return [m.display for m in STT_MODELS]


def chat_dropdown_labels() -> list[str]:
    return [m.display for m in CHAT_MODELS]
...
def stt_options_for_ui(selected_id: str) -> tuple[list[str], list[str], str]:
    ...
    labels = [m.display for m in options]
    ids = [m.id for m in options]
    label = next((m.display for m in options if m.id == selected_id), labels[0])
