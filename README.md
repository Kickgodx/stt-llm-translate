...
Два режима: **Пакетная запись** и **Лайв (по паузе)**. Источник звука: **микрофон** или **системный звук** (Windows, WASAPI loopback). Модели STT и LLM — выпадающие списки в UI.
...
### Русская речь с английскими терминами (Python, SDET, …)

1. **STT** — в UI поле «Язык» оставьте `ru` или пустым (авто); при `ru` Whisper чаще держит русский, но может искажать английские слова.
2. **`OPENROUTER_GLOSSARY`** — ваши частые термины через запятую; подмешиваются в подсказку STT и в промпты LLM.
3. **`OPENROUTER_STT_PROMPT`** — полная подсказка для Whisper (если провайдер её принимает).
4. **Перевод / промпт LLM** — встроенная инструкция сохранять техтермины латиницей и не транслитерировать.
